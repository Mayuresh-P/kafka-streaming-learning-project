...
public class GreetingResource {
    ...
    public String startStreams(){
        ...
        final GlobalKTable<Integer, JsonNode> customers= builder.globalTable(CUSTOMER_TOPIC, Materialized.<Integer, JsonNode>as(storeSupplier).withKeySerde(Serdes.Integer()).withValueSerde(new JSONSerde()));

        final KStream<Integer, JsonNode> salesStream = builder.stream(SALES_TOPIC, Consumed.with(Serdes.Integer(),new JSONSerde()));

        final KStream<Integer,JsonNode> customerSales = builder.stream(SALES_CUSTOMER_TOPIC, Consumed.with(Serdes.Integer(),new JSONSerde()));
